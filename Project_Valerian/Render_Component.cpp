...
void Simple_Clip_Tile_Renderer::Draw()
{
	...
	SDL_Rect pos_rect = { (tile_specs.x*camera.w) + SCREEN_WIDTH/2 + camera.x, (tile_specs.y*camera.w) + SCREEN_HEIGHT/2 + camera.y, tile_specs.w*camera.w, tile_specs.h*camera.w };
...
void Multi_Clip_Tile_Renderer::Check_Bus_For_Surrounding_Tile_Updates()
{
	for (int i = 0; i < service_locator->get_MB_Pointer()->count_SG_tile_map_update_messages; i++)
	{
		...
		int delta_x = message_tile_x - tile_specs.x;
		int delta_y = message_tile_y - tile_specs.y;
